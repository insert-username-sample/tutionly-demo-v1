...
const WaitlistPage: React.FC = () => {
  ...
  const handleAnswer = (answer: string, autoAdvance: boolean = true) => {
    setAnswers(prev => ({
      ...prev,
      [currentQ.id]: answer
    }));

    // Only auto-advance for multiple choice questions and required fields with content
    if (autoAdvance && currentQ.required && currentQ.type === 'multiple_choice' && answer.trim()) {
      setTimeout(() => {
        if (currentQuestion < questions.length - 1) {
          setCurrentQuestion(currentQuestion + 1);
        }
  ...
  // Handle text input changes without auto-advancing
  ...
  // Handle text input key events (for Enter to advance)
  const handleTextKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && ((answers[currentQ.id as keyof WaitlistFormData] as string)?.trim())) {
      handleNext();
    }
  };
